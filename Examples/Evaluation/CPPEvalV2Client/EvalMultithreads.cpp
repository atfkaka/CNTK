...
void RunEvaluation02OneConv(FunctionPtr, const DeviceDescriptor&);
...
/// All cloned functions share the same parameters.
/// </description>
void MultiThreadsEvaluationFor02OneConv(const DeviceDescriptor& device, const int threadCount)
{
    // The model file will be trained and copied to the current runtime directory first.
    auto modelFuncPtr = CNTK::LoadLegacyModel(DataType::Float, L"02_OneConv", device);
    // auto modelFuncPtr = CNTK::LoadLegacyModel(DataType::Float, L"Malta-Proto1-D2-50-I5.cntk.bin", device);    

    outputFunctionInfo(modelFuncPtr);
    fprintf(stderr, "MultiThreadsEvaluationFor02OneConv on device=%d\n", device.Id());

    // Run evaluation in parallel.
    std::vector<std::thread> threadList(threadCount);
    for (int th = 0; th < threadCount; ++th)
    {
        threadList[th] = std::thread(RunEvaluation02OneConv, modelFuncPtr->Clone(), device);
    }
...
void RunEvaluation02OneConv(FunctionPtr evalFunc, const DeviceDescriptor& device)
{
    const std::wstring inputNodeName = L"features";
    const std::wstring outputNodeName = L"ol_output";
...
{
    MultiThreadsEvaluationFor02OneConv(DeviceDescriptor::CPUDevice(), 1);
