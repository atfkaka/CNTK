...
/// 
/// This is an example to illustrate how users would use sparse input.
/// Here we assume the tensor is 30x30x30, and the tensorData is the data in dense format.
/// Todo: How can a user provide the n-dimensional tensor data in sparse format? Ideally he does not need
/// first to flat the tensor into 1-dimensional vector.
///
void SpaseInputEvalSample1(vector<float[30][30][30]> tensorData, int numOfSequences, vector<int> numOfSamples)
{
    ...
    for (int seq = 0; seq < numOfSequences; seq++)
    {
        ...
        {
            count = 0;
            // first transform the tensor data into sparse presentation
            for (int i = 0; i < 30; i++)
                for (int j = 0; j < 30; j++)
                    for (int k = 0; k < 30; k++)
                    {
...
///
void SpaseInputEvalSample2(vector<float[30][30][30]> tensorData, int numOfSequences, vector<int> numOfSamples)
{
    // This is a sample to illustrate how to use Value::Create interface for sparse input.
    // Assumging the tensor data is 30x30x30.        
    int count;
    ...
    // Prepare sparse presentation 
    SparseBatchData<float> inputData;
    inputData.numOfSamples = numOfSamples;
    inputData.numOfSequences = numOfSequences;
            ...
            for (int i = 0; i < 30; i++)
                for (int j = 0; j < 30; j++)
                    for (int k = 0; k < 30; k++)
                    {
                        if (tensorData[numTensor][i][j][k] != 0)
                        {
                            inputData.m_data.push_back(tensorData[numTensor][i][j][k]);
                            inputData.m_indices.push_back(i * 30 + j * 30 + k);
                            count++;
                        }
                    }
            inputData.m_nnzCounts.push_back(count);
            numTensor++;
        }
    }
