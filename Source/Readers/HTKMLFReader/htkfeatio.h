...
class htkmlfreader : public std::vector<std::pair<string, vector<ENTRY>>> // [key][i] the data
{
                    ...
                    const double htkTimeToFrame)
    {
        ...
        this->push_back(std::make_pair(regex_replace(filename, regex("\\.[^\\.\\\\/:]*$"), string()), std::vector<ENTRY>()));
