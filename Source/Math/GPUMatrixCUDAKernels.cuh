...
__global__ void _denseMulSparseCSCTransposeToSparseBlockCol(
    ...
    const ElemType* lhsValues,
    const size_t numRowsLhs,
    const size_t numColsRhs,                // The number of columns of rhs before transpose. I.e. it is the 'middle' dimension in the matrix product
    const ElemType* rhsNZValues,
    const GPUSPARSE_INDEX_TYPE* rhsRows,    // Mapping the nnz id of the values to row index 
    const GPUSPARSE_INDEX_TYPE* rhsCols,    // Start id of each column.
    const GPUSPARSE_INDEX_TYPE* rhsRowIdx,  // Each nonzero row of the sparse matrix get's an index (call it block id). This array (size nnz) maps the nz-value row to the corresponding block-id.
    ElemType* resultValues,                 // Modified to contain values of the product
    GPUSPARSE_INDEX_TYPE* blockId2Col       /* Maps block ids to to colum of the result matrix.*/)
{
    const CUDA_LONG index = blockIdx.x * blockDim.x + threadIdx.x;
    const CUDA_LONG lhsCol = index / numRowsLhs; // rhsCol == lhsCol
    if (lhsCol >= numColsRhs)
